...
gui.elements = {
    ...
    min_targets_to_attack   = slider_int:new(0, 10, 1, get_hash(plugin_label .. '_min_targets_to_attack')),
...
gui.render = function()
    ...
    if gui.elements.run_tree:push('Run Settings') then
        ...
        gui.elements.min_targets_to_attack:render('Min nearby targets to attack', 'Requires "Disable rotation while rushing" ON. Set to 0 to fully suppress UR during rush. Set >= 1 to attack only when that many enemies are within 5m.')
